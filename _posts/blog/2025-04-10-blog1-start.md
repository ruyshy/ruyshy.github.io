...
{% img "github-blog-visual-studio-code-screenshoort.png" %}
...
{% img "github-blog-visual-studio-code-file-list.PNG" %}

해당 폴더가 열리면서 왼쪽에 파일 리스트가 열립니다. 수정할 파일을 열어서 편집하거나, 마우스 오른쪽 클릭하여 파일을 추가/삭제/이름 수정 작업을 진행할 수 있습니다.

{% img "github-blog-visual-studio-code-extension.png" %}
...
`Material Icon Theme`은 Visual Studio Code에서 파일 및 폴더 아이콘을 Google의 Material Design 스타일로 바꾸어 개발 환경을 더욱 직관적이고 세련되게 만들어주는 인기 확장 프로그램입니다.

{% cardlink https://marketplace.visualstudio.com/items/?itemName=PKief.material-icon-theme %}
...
{% img "github-blog-visual_studio_code_prettier_code_formatter_setting1.png", "visual_studio_code_prettier_code_formatter_setting1" %}

위에 빨간 박스를 클릭하여 `Prettier - Code formatter`을 찾아 초록 박스를 클릭 후, 설정을 클릭 해서 설정 창으로 이동합니다.

{% img "github-blog-visual_studio_code_prettier_code_formatter_setting2.png", "visual_studio_code_prettier_code_formatter_setting2" %}

1. 상단 검색 창(빨간 박스)에 `format on save`을 입력합니다.
2. 노란 박스 부분의 옵션을 체크 해줍니다.
3. 해당 기능은 파일 저장`Ctrl+S`을 할 때 마다 코드를 이쁘게 정렬 해주는 옵션입니다.

{% img "github-blog-visual_studio_code_prettier_code_formatter_setting3.png", "visual_studio_code_prettier_code_formatter_setting3" %}

1. 상단 검색 창에 `Default Formatter`을 검색합니다.
2. 설정이 prettier format (esbenp.prettier-vscode) 로 설정되어 있는 지 확인하고 안되어 있다면 `Prettier - Code formatter` 해당 확장을 선택해 줍니다.

{% cardlink https://marketplace.visualstudio.com/items/?itemName=esbenp.prettier-vscode %}
...
{% img "github-blog-markdown-click.png" %}

해당 버튼을 클릭하면 markdown 문법이 적용된 것을 미리보기를 열 수 있습니다.

{% cardlink https://marketplace.visualstudio.com/items?itemName=starkwang.markdown %}
...
{% cardlink https://marketplace.visualstudio.com/items/?itemName=yzhang.markdown-all-in-one %}

{% img "github-blog-markdown-preview.png" %}

{% raw %}`{% %}`{% endraw %} 해당 문법은 preview에 적용되지 않긴하지만, markdown 문법들은 적용되서 괜찮은거 같습니다.
...
웹사이트 탭에 표시되는 작은 아이콘입니다. `.ico`, `.png`, `.svg` 등의 포맷을 사용할 수 있지만 브라우저 호환성을 위해 `favicon.ico`가 가장 일반적입니다.

설정 방법 (Jekyll + GitHub Pages 기준)

1. `favicon.ico` 파일 준비
   - `32x32` 또는 `16x16` 크기의 `.ico` 포맷 이미지 파일을 준비하세요.
   - 변환 사이트 예:
...
3. `_includes/head.html` 또는 `_layouts/default.html`에 링크 추가
...
보통 `minimal-mistakes` 테마에서는 `_includes/head/custom.html`에 추가하면 안전합니다.

1. GitHub Actions를 사용하는 경우 `_site/`에 복사되도록 확인
   - Jekyll 빌드시 `favicon.ico`가 `/_site/`에 복사되어야 합니다.
   - `_config.yml`에 아래 설정이 있다면, favicon도 복사될 수 있도록 확인:
...
GitHub Pages에 배포 후, 브라우저에서 `https://yourusername.github.io/favicon.ico`가 열리는지 확인
