...
- A. rax 값을 rbx로 바꿈
- B. rbx 값을 rax에서 뺀 뒤 저장
- C. 두 값을 비교하고 플래그만 설정 [correct]
- D. rax와 rbx를 교환
...
- A. rax 값을 스택에 저장하고 RSP는 증가
- B. rax 값을 스택에 저장하고 RSP는 감소 [correct]
- C. 스택에서 값을 꺼내 rax에 저장
- D. rax가 스택 포인터로 바뀜
...
- A. rbx에 스택의 맨 위 값이 들어가고 RSP는 증가 [correct]
- B. rbx에 스택의 맨 위 값이 들어가고 RSP는 감소
- C. rbx의 값이 스택에 들어감
- D. rbx와 rsp를 교환함
...
- A. 현재 위치를 저장하고 func로 점프 [correct]
- B. func의 결과를 rax에 저장
- C. 함수를 종료시킴
- D. func를 push함
...
- A. rax를 초기화
- B. 함수 인자를 정리
- C. 스택에서 return 주소를 꺼내 복귀 [correct]
- D. 프로그램 종료
