...
`Assembly` 언어로 `Hello World` 출력부터 덧셈 계산기, 사칙연산 계산기까지 `NASM`과 `io64.inc` 매크로를 활용하여 실습해봅니다. 이 예제들을 통해 `PRINT_STRING`, `READ_INT`, `cmp`, `call`, `ret` 등의 명령어 흐름과 스택 사용법까지 익힐 수 있습니다.
...
| `mov rbp, rsp`     | 현재 스택 포인터를 `RBP`에 저장해서 스택 프레임 설정 |
| `xor rax, rax`     | `RAX`를 0으로 초기화 (프로그램 정상 종료 코드)       |
| `ret`              | 호출한 곳으로 복귀 (여기선 `OS`에 리턴)              |
